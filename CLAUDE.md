...
6. **링크 옆 라벨**: 책 카드·CTA 버튼 자체에 `[광고]` 또는 `[AD]` 보조 라벨 (가시성 확보된 색·크기)

## 컴포넌트 설계 가이드 (구현 시 강제)

- **`AffiliateNotice`** 컴포넌트 신설:
  - prop: `variant: 'top' | 'inline'`
  - 색상: `var(--text-secondary)` 이상의 가시성 (회색 muted 금지)
  - 폰트: 본문 텍스트(0.86~1rem) 이상
  - 박스: 옅은 배경 + 가는 테두리로 시각 분리
- **`BookRecommendBlock`** 등 affiliate 카드:
  - 카드마다 `[광고]` 보조 라벨
  - 카드 그룹 위에 `<AffiliateNotice variant="inline" />` 자동 삽입
- **페이지 단위**: affiliate 링크가 1개 이상 존재하면 **페이지 상단에 `<AffiliateNotice variant="top" />` 자동 삽입**
