...
```
{name} ({code}) ETF | 구성종목·분배금·주가 매일 갱신
```
이유: "ETF" 키워드 = "{ETF명} ETF" 검색 직접 매칭. "매일 갱신" = freshness 신호.
※ 2026-06-11: KIS(한국투자증권) 실시간 시세 중단 → data.go.kr 일별(종가) 운영으로 전환. "실시간 갱신"은 사실과 달라 "매일 갱신"으로 정정(정직성·AdSense). 데이터는 DataMiner가 매일 수집하므로 freshness 신호 유지.
...
```
1. 시세 및 수익률 ({YYYY-MM-DD} 종가 기준)   — 시세 있을 때 (KRX 일별 종가)
