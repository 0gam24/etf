...
export interface GuideDef {
  ...
  section: '월배당 가이드' | '커버드콜 가이드' | '테마 ETF 가이드' | '은퇴 자산 가이드' | '배당주 가이드' | '환헤지 가이드' | '해외상장 가이드' | 'ETF 입문 가이드' | 'ETF 세금 가이드' | 'ETF 비용 가이드' | 'ETF 비교 가이드' | 'ETF 분배금 가이드' | 'ETF 적립식 가이드' | 'ETF 리밸런싱 가이드' | 'ETF 괴리율 가이드' | 'ETF 레버리지 가이드';
  /**
