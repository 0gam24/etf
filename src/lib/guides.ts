...
export interface GuideDef {
  ...
  section: '월배당 가이드' | '커버드콜 가이드' | '테마 ETF 가이드' | '은퇴 자산 가이드' | '배당주 가이드' | '환헤지 가이드' | '해외상장 가이드' | 'ETF 입문 가이드' | 'ETF 세금 가이드' | 'ETF 비용 가이드' | 'ETF 비교 가이드' | 'ETF 분배금 가이드' | 'ETF 적립식 가이드' | 'ETF 리밸런싱 가이드' | 'ETF 괴리율 가이드' | 'ETF 레버리지 가이드' | 'ETF 채권 가이드' | 'ISA 계좌 가이드' | 'ETF 매매 가이드' | 'ETF 금 가이드';
  /**
...
export const GUIDE_CLUSTERS: GuideCluster[] = [
  ...
  {
    title: '배당·인컴',
    description: '월배당·커버드콜·미국배당 등 현금 흐름 중심 ETF와 분배금 이해.',
    slugs: ['monthly-dividend', 'covered-call', 'us-dividend', 'etf-dividend'],
  },
  {
    title: '테마·섹터',
