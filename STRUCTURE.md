...
- 2026-05-12 — Phase 6 메인 페이지 실시간 통합 (사용자 명시 "모든 chapter 실시간 + 자정 boundary") ★★★
  ...
  - **HERO 우측 카드 전체 라이브 swap** (commit 9694de3 — HeroFeaturedLabel 대체): `LiveHeroFeatured` client — HomeHeroV3 우측 aside 전체를 client 화. baseline (top10) + heroDict (각 ETF code 별 holdings·catalyst pre-fetched) 받아 30s polling. 양수 max 자동 swap → 코드·이름·가격·등락률·거래량·LIVE pill·holdings·catalyst 모두 동기화. 전 종목 음수 → 거래량 1위 폴백 + 라벨 분기. HomeHeroV3 server component 유지하면서 우측만 client 위임. page.tsx `getEtfHoldings` import + heroDict 사전 구성 (top10 × holdings.slice(0,3) + breaking glob).
