...
async function buildWeeklyReport(etfData) {
  ...
  const prompt = `당신은 ${persona.name} — ${persona.title} (AI 분석 모델)입니다.
모델 설명: ${persona.modelDescription || persona.title}
데이터 출처: ${(persona.dataSources || []).join(', ')}
...
async function buildStockMaster(etfData) {
  ...
  const prompt = `당신은 ${persona.name} — ${persona.title} (AI 분석 모델)입니다.
모델 데이터 출처: ${(persona.dataSources || []).join(', ')}
${persona.methodology ? `방법론: ${persona.methodology}` : ''}
...
- 본문 어딘가에 [${target.name} 종목 사전](/etf/${target.code.toLowerCase()}) 형태의 내부 링크 1회 자연 삽입 (실시간 시세 확인 유도).
- 마지막 줄: "${persona.closingSignature}"
`;

  const text = await geminiWrite(prompt);
  if (!text) return null;
  // /etf 내부 링크가 누락됐으면 결론 직전 자동 삽입 (체류시간·내부 링크 그래프 강화)
  let content = text.trim();
  if (!/\[.*?\]\(\/etf\//.test(content)) {
    const tickerLower = target.code.toLowerCase();
    const linkLine = `\n> 📊 ${target.name}의 실시간 시세·구성종목·분배 정보는 [${target.name} 종목 사전](/etf/${tickerLower})에서 확인하실 수 있습니다.\n`;
    // 마지막 H2 섹션 직전 또는 본문 끝에 삽입
    const lastH2 = content.lastIndexOf('\n## ');
    if (lastH2 > 0) {
      content = content.slice(0, lastH2) + linkLine + content.slice(lastH2);
    } else {
